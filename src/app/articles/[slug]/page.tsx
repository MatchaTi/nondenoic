...
export default function PostLayout({ params }: { params: { slug: string } }) {
  ...
  return (
    <article className='mt-20 w-full border-y border-neutral-200 lg:mb-28'>
      <section className='mx-auto w-full max-w-7xl p-4 px-8'>
        ...
        <div className='w-full grid-cols-3 lg:grid'>
          <div className='order-1 mb-10 sm:px-4 lg:col-span-2'>
            <h1 className='text-dark-headings mb-2 text-lg font-bold sm:text-3xl'>{post.title}</h1>
          ...
          <div className='z-10 order-2 sm:px-4 lg:sticky lg:top-20 lg:px-6'>
            <div className='border-light-accent/5 mb-4 border-b pb-4'>
              <div className='text-dark-headings text-lg font-semibold'>{post.author}</div>
              <time className='text-sm opacity-80'>Diposting {time(post.date)}</time>
            </div>
            <h2 className='text-dark-headings mb-4 text-xl font-semibold'>Artikel Lainnya</h2>
            <ul>
